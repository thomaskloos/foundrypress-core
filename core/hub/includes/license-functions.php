 ...
 *
 * Distributed with the portable FoundryPress install.
 * These functions:
 * - activate a license against the remote API
 * - cache local license data
 * - re-check the license weekly
 * - protect licensed areas of the app
 */
...
if (!defined('FP_LICENSE_RECHECK_INTERVAL')) {
    define('FP_LICENSE_RECHECK_INTERVAL', 7 * 24 * 60 * 60); // 7 days
}

if (!defined('FP_LICENSE_FAIL_GRACE')) {
    define('FP_LICENSE_FAIL_GRACE', 14 * 24 * 60 * 60); // 14 days
}
...
if (!function_exists('fp_activate_remote_license')) {
    function fp_activate_remote_license(string $licenseKey): array
    {
        ...
        $payload = [
            ...
            'version'     => defined('FP_VERSION') ? (string) FP_VERSION : '',
        ];
...
if (!function_exists('fp_validate_license_or_redirect')) {
    function fp_validate_license_or_redirect(string $redirectUrl = '/hub/license.php'): void
    {
        $status = fp_get_license_status();
...
if (!function_exists('fp_require_valid_license')) {
    ...
    {
        fp_validate_license_or_redirect('/hub/license.php');
    }
}
